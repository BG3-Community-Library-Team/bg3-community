...
>
> • ***Doesn't clutter UI***: MCM centralizes settings for all mods that use it, so you don't have to worry about cluttering players' screens with yet another IMGUI window, thinking when should it initialize, activate, or even about keybindings - and possible conflicts thereof - for  showing such windows. Users will have a familiar (Noesis) button for opening MCM.
> • Furthermore, you can conditionally hide elements of your configuration based on settings' values with `VisibleIf` entries.
>
...
On top of it all, if you currently offer multiple mod versions with different code to avoid dealing with the complexities of providing settings, you can simplify this by using a single version with MCM to introduce settings. This way, you can avoid creating and maintaining several different .pak releases for your mods.
...
> It's extremely recommended to define BG3MCM as a dependency in your meta.lsx file. It allows you to set dependencies and their minimum required versions. Always set the required version (`Version64`) of MCM to the version you're using during the development of your mod. [Example of listing dependencies in a meta.lsx file](https://github.com/AtilioA/BG3-auto-send-food-to-camp/blob/46822bb0a8af4db524fd1ccb6f8a277724f5630c/Auto%20Send%20Food%20To%20Camp/Mods/AutoSendFoodToCamp/meta.lsx#L7-L24 'Auto Send Food To Camp');
> • This allows mod managers to ***ensure*** that MCM is loaded ***before** your own mod* - eliminating the need to instruct users to do so manually and avoiding incorrect reports/troubleshooting when they don't!
{.is-warning}
