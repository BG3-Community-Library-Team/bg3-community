...
>
> • ***Doesn't clutter UI***: MCM centralizes settings for all mods that use it, so you don't have to worry about cluttering players' screens with yet another IMGUI window, thinking when should it initialize, activate, or even about keybindings - and possible conflicts thereof - for  showing such windows. Users will have a familiar (Noesis) button for opening MCM. Furthermore, you can conditionally hide elements of your configuration based on settings' values with `VisibleIf` entries.
>
